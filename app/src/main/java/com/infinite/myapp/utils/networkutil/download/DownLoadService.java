...
import com.infinite.myapp.utils.networkutil.download.service.MyPersistentService;
import com.infinite.myapp.utils.networkutil.download.service.MyServiceManager;
import com.infinite.myapp.utils.networkutil.download.service.MyTaskService;
...
public class DownLoadService implements IDownLoad {
    ...
    @Override
    public void onDownLoadList(final String download_path, final ArrayList<DownLoadModel> urls, final IDownLoadCallback callback) {
        list = new ArrayList<>();
        mCallback = callback ;
        new Thread(new Runnable() {
            ...
            public void run() {
                Looper.prepare();
                download(download_path,urls,callback);
                Looper.loop();
            }
        }).start();
    }
    ...
    private void download(String download_path,final ArrayList<DownLoadModel> urls, final IDownLoadCallback mCallback){
