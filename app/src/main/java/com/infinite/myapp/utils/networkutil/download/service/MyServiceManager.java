    ...
    public void execute(){
        ...
        if(isNormalService){ // 是否是普通的Service
            ...
            if(isNeedBackgroundDownload){ // 需要后台下载
                Intent intent = new Intent(MyNetWorkRequest.getInstance().sContext,MyPersistentService.class);
...
//                intent.putExtra(MyTaskService.DOWNLOAD_FILE_SAVE_PATH,savePath);
                ...
                MyPersistentService.startDownloadFileTask(MyNetWorkRequest.getInstance().sContext,intent ,resultReceiver);
            }else{ // 不需要后台下载
                if (url == null || /*url.length == 0 || */TextUtils.isEmpty(savePath)) {
                    throw new IllegalArgumentException("下载文件的url 或者 下载文件存储路径 savePath 为空 ") ;
                }else{
                    Intent intent = new Intent(MyNetWorkRequest.getInstance().sContext,MyTaskService.class);
                    intent.putExtra(MyTaskService.DOWNLOAD_FILE_URL,url);
                    intent.putExtra(MyTaskService.DOWNLOAD_FILE_SAVE_PATH,savePath);
                    MyTaskService.startDownloadFileTask(MyNetWorkRequest.getInstance().sContext,intent ,resultReceiver);
                }
            }
        }
    }
