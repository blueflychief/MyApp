 ...
 */
package com.infinite.myapp.utils.nohttp;

import android.content.Context;
import android.content.DialogInterface;

import com.infinite.myapp.utils.ToastUtils;
import com.infinite.myapp.widget.WaitDialog;
...
//                context.showWebDialog(response);
    ...
    public void onFailed(int what, String url, Object tag, Exception exception,
        ...
        if (exception instanceof NetworkError) {// 网络不好
            ...
            ToastUtils.showToast(context, "请求超时，网络不好或者服务器不稳定。");
        } else if (exception instanceof UnKnownHostError) {// 找不到服务器
            ToastUtils.showToast(context, "未发现指定服务器。");
        } else if (exception instanceof URLError) {// URL是错的
            ToastUtils.showToast(context, "URL错误。");
        } else if (exception instanceof NotFoundCacheError) {
            // 这个异常只会在仅仅查找缓存时没有找到缓存时返回
            ToastUtils.showToast(context, "没有发现缓存。");
        } else if (exception instanceof ProtocolException) {
            ToastUtils.showToast(context, "系统不支持的请求方式。");
        } else {
